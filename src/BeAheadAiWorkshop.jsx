...
function EarlyBirdTimer({ compact = false }) {
  ...
  const remaining = Math.max(0, earlyBirdEndsAt - now);
  ...
  if (!active) return <span className={compact ? 'ba-timer-compact' : 'ba-timer'}>EARLY-BIRD PRICE ENDED · LIVE SEAT ₹200</span>;
  return <span className={compact ? 'ba-timer-compact' : 'ba-timer'}><b>₹150 PRICE ENDS IN</b> <strong>{hours}:{minutes}:{seconds}</strong> <small>then ₹200</small></span>;
}
  ...
  return <div className="beahead-page">
    ...
    <main>
      <section className="ba-hero" id="top">
        ...
        <div className="ba-hero-grid">
          <div>
            ...
            <div className="ba-hero-credibility" style={{ marginTop: 19, display: 'grid', gap: 8, fontSize: 13, lineHeight: 1.35 }}><span><b style={{ color: '#ff7a16' }}>→</b> Get a BeAhead workshop certificate for your resume</span><span><b style={{ color: '#ff7a16' }}>→</b> Learn from Shanjai Raj — built products with seven-figure revenue</span><span><b style={{ color: '#ff7a16' }}>→</b> Be ahead before AI becomes a basic job requirement</span><span><b style={{ color: '#ff7a16' }}>→</b> Learn the tools Shanjai has tested for 1,000+ hours</span></div>
          </div>
          <div className="ba-hero-media"><MediaSlot src="/images/shanjai-raj-workshop.jpg" label="Shanjai Raj, BeAhead workshop instructor"/><div className="ba-float ba-float-one"><Wand2/><b>From manual work</b><span>to AI-assisted work</span></div><div className="ba-float ba-float-two"><Zap/><b>One live session</b><span>practical workflows</span></div></div>
        </div>
      ...
      <section className="ba-section ba-workplace-shift" style={{ background: '#f7f6f2', color: '#151819' }}>
        ...
        <p className="ba-source-note">Company names are shown only as sourced public-workplace context. They do not endorse, sponsor or partner with BeAhead.</p>
      ...
      <section id="beahead-enrol" className="ba-section ba-offer"><div className="ba-offer-card"><div><p className="ba-kicker">YOUR LIVE WORKSHOP INVITATION</p><h2>This Sunday, give yourself a stronger way to work from next week.</h2><p>Join Shanjai live. Learn the tools. Follow the steps. Start using AI in your work from Monday.</p><div className="ba-invitation-details"><span><b>DATE</b>Sunday, 9 August 2026</span><span><b>TIME</b>3:00 PM–6:00 PM IST</span><span><b>FORMAT</b>Live online · Simple English</span><span><b>PRICE TODAY</b>₹150 early-bird seat</span></div><div className="ba-includes"><span><Check/> 3-hour live Claude & AI workshop</span><span><Check/> Learn prompting that gets useful results</span><span><Check/> See how to turn an idea into a simple app</span><span><Check/> BeAhead workshop certificate</span></div></div><aside><small>One-time workshop access</small><strong>₹150</strong><span>Early-bird price · ₹200 after midnight</span><CTA>Register for Sunday · ₹150</CTA><p>Secure payment and joining details are the next step.</p></aside></div></section>
    ...
    <footer className="ba-footer"><a href="#top" className="ba-brand">Be<span>Ahead</span></a><p>Simple AI skills for people who want to stay ahead.</p><a href="/#/build-app-with-ai">View the AI App-Building Workshop →</a><span><a href="/terms.html">Terms</a> · <a href="/privacy.html">Privacy</a> · <a href="/refunds.html">Refunds</a></span></footer>
