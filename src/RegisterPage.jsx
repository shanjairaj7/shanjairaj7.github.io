...
function Countdown() {
  ...
  return <div className="rp-countdown"><span>{active ? 'EARLY-BIRD ₹150 PRICE ENDS IN' : 'EARLY-BIRD PRICE ENDED'}</span>{active ? <strong>{parts.join(':')}</strong> : <strong>₹200</strong>}<small>{active ? 'After this, the live seat is ₹200.' : 'Standard live-seat price.'}</small></div>;
}
...
export default function RegisterPage() {
  ...
  const price = active ? 150 : 200;
  const priceLabel = active ? 'Today’s early-bird price' : 'Current live-seat price';
  const submit = event => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    window.sessionStorage.setItem('beaheadRegistration', JSON.stringify({
  ...
  return <main className="register-page">
    <header className="rp-top"><a href="/" aria-label="Back to BeAhead"><ArrowLeft size={18}/> Back to workshop</a><a className="rp-brand" href="/">Be<span>Ahead</span></a></header>
    ...
    <div className="rp-layout">
      <section className="rp-form-card" id="registration-form" ref={formCardRef}><div className="rp-card-title"><p>BEAHEAD LIVE CLAUDE & AI WORKSHOP</p><h2>Reserve your live seat</h2><span>Fill this in. Secure payment is the next step.</span></div><Countdown/>
      ...
      <aside className="rp-summary"><p>YOUR LIVE WORKSHOP INVITATION</p><h2>Sunday, 9 August</h2><div className="rp-invite"><span><b>WHEN</b>3:00 PM–6:00 PM IST</span><span><b>WHERE</b>Live online workshop</span><span><b>WITH</b>Shanjai Raj</span><span><b>YOU WILL GET</b>Live workshop + 2 practical AI sessions + certificate</span></div><div className="rp-value"><p>WHAT YOU WILL LEARN LIVE</p><span><Check/> Prompt AI to get the result you want</span><span><Check/> Use Claude, Gemini, Perplexity and Codex for real work</span><span><Check/> Turn one useful idea into a simple app</span><span><Check/> See how an app can take payments and be shared online</span><span><Check/> BeAhead workshop certificate</span></div><div className="rp-price" style={{ padding: 16 }}><span>{active ? <>Standard live-seat price <s>₹200</s></> : 'Standard live-seat price'}</span><b style={{ marginTop: 8, fontSize: 17 }}>Pay today: ₹{price}</b>{active && <em style={{ marginTop: 5 }}>You save ₹50 today</em>}</div><p className="rp-fineprint"><Mail size={14}/> Workshop joining details will be sent after payment is connected and completed.</p></aside>
    </div>
  </main>;
}
