...
const CTA = ({ label = 'Save your seat now' }) => <button className="cta" onClick={() => { track('cta_clicked', { cta: label }); scrollToOffer(); }}>{label} <ArrowRight size={18}/></button>;
...
function AppIdeaWorkshop() {
 ...
 <main>
  <section data-track-section="app-hero" className="hero section">
   <nav><a className="brand" href="#top">Made <span>for More</span></a><div className="nav-proof"><Users size={16}/> Built for people with ideas</div></nav>
...
function Router() {
  ...
  useEffect(() => {
    ...
    return () => { window.clearTimeout(timer); stopObserving?.(); };
  }, [route]);
